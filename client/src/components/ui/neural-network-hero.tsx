...
// Note: SplitText is a premium GSAP plugin. Using a fallback for text animation.
// If you have GSAP Club membership, you can import it: import { SplitText } from 'gsap/SplitText';
...
function ShaderBackground() {
  ...
  return (
    <div ref={canvasRef} className="bg-orange-900 absolute inset-0 -z-10 w-full h-full" aria-hidden>
      <Canvas
        camera={camera}
      ...
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-orange-950/40 via-transparent to-orange-900/30" />
    </div>
  );
}
...
export default function Hero({
  ...
  useGSAP(
    () => {
      ...
      // SplitText is a premium GSAP plugin, so we'll animate the header directly
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
        ...
        <div ref={ctaRef} className="flex flex-wrap items-center gap-3 pt-2">
          {ctaButtons.map((button, index) => (
            <a
              key={index}
              href={button.href}
              className={`rounded-2xl border border-white/10 px-5 py-3 text-sm font-light tracking-tight transition-colors focus:outline-none focus:ring-2 focus:ring-white/30 duration-300 !text-white ${
                button.primary
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-24 bg-gradient-to-t from-orange-950/60 to-transparent" />
    </section>
  );
}
