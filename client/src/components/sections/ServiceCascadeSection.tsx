  ...
  useEffect(() => {
    const unsubscribe = scrollProgress.on("change", (latest) => {
      const newIndex = Math.round(
        Math.max(0, Math.min(latest, totalItems - 1)),
      );
    ...
    >
      ...
      <div className="sticky top-0 h-screen flex items-center justify-center bg-black z-10 pt-12 sm:pt-16 md:pt-20 lg:pt-24 relative">
        <div className="container mx-auto px-4 sm:px-6 md:px-8">
          ...
          <div
            className={`grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 md:gap-10 lg:gap-12 xl:gap-16 items-center px-4 sm:px-6 md:px-8 lg:px-0 ${
          ...
          >
            ...
            <div
              className={`space-y-6 lg:space-y-8 flex flex-col justify-center items-start ${
                      ...
                      {service.items.map((_, index) => (
                        ...
                        >
                          <motion.div
                            ...
                            style={{
                              width: `${100 / service.items.length - 2}px`,
                            }}
                            animate={{
                              backgroundColor:
                                index <= activeIndex ? "#FF5A00" : "#374151",
                            }}
...
export default function ServiceCascadeSection() {
  ...
  const totalHeight = services.reduce(
    (acc, service) => acc + service.items.length * 100,
    0,
  );
  ...
  return (
    <div className={`relative`} style={{ height: `${totalHeight}vh` }}>
      {services.map((service, serviceIndex) => {
        const sectionStart = services
          .slice(0, serviceIndex)
          .reduce((acc, s) => acc + s.items.length * 100, 0);
